...
testMask_dir = '../../dataSets/reference.csv' ## Point to the Camelyon test set mask location
...
parser.add_argument('--num_cls', default=5, type=int)
parser.add_argument('--data_dir', default='../autodl-tmp/Aptos_avg16', type=str)
parser.add_argument('--numGroup', default=3, type=int)
...
def main():
    ...
    log_dir = os.path.join(params.log_dir, 'log-Aptos-cvpr.txt')
    ...
    for ii in range(params.EPOCH):
        ...
        auc_val = test_attention_DTFD_preFeat_MultipleMean(classifier=classifier, dimReduction=dimReduction, attention=attention,
                                                           UClassifier=attCls, mDATA_list=(SlideNames_train, FeatList_train, Label_train), criterion=ce_cri, epoch=ii,  params=params, f_log=log_file, writer=writer, numGroup=params.numGroup_test, total_instance=params.total_instance_test, distill=params.distill_type)
        print_log(' ', log_file)
...
def test_attention_DTFD_preFeat_MultipleMean(mDATA_list, classifier, dimReduction, attention, UClassifier, epoch, criterion=None,  params=None, f_log=None, writer=None, numGroup=3, total_instance=3, distill='MaxMinS'):
            ...
            for tidx, tfeat in enumerate(batch_feat):
                ...
                for jj in range(params.num_MeanInference):
                    ...
                    for tindex in tfeat:
                        ...
                        tindex = tindex.to(params.device)
                        tmidFeat = dimReduction(tindex)
    ...
    macc_0, mprec_0, mrecal_0, mspec_0, mF1_0 = eval_metric(gPred_0, gt_0)
    ...
    writer.add_scalar(f'macc_0 ', macc_0, epoch)
    writer.add_scalar(f'macc_1 ', macc_1, epoch)
